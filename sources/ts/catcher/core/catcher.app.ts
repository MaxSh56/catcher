...
import { CatcherAppConfig } from "./lib";
...
export class CatcherApp {
    private static _singleInstance: CatcherApp;
    ...
    constructor() {
        if (CatcherApp._singleInstance) {
            throw Error("CatcherApp can't be initialised twice");
        }

        const appSize: number = CatcherApp._appSize;
    ...
    private handleOnClickView(event: MouseEvent): void {
        ...
        const appSize: AppSize = CatcherApp.appSize;
    ...
    public static getSingle(): CatcherApp  {
        if (!CatcherApp._singleInstance) {
            CatcherApp._singleInstance = new CatcherApp();
        }
        return CatcherApp._singleInstance;
    }

    public init(options: CatcherAppConfig): void {
        const {
            parentElement,
            playerInitConfig,
    ...
    public static get appSize(): AppSize {
        return new AppSize(CatcherApp._appSize, CatcherApp._appSize);
    }
}